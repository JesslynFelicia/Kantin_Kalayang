<template>
  ...
  <q-page class="q-pa-sm flex-center bg-grey-3">
    ...
    >
      <div class="text-subtitle1" style="font-weight: 600; margin-top: 20px">
...
export default {
  ...
  methods: {
    ...
    viewTransaksi() {
      axios
        .post("http://127.0.0.1:8000/api/riwayatpenjual", {
        ...
        .then((response) => {
          ...
          response.data.data.forEach((data, index) => {
            console.log(
              `formatted_tanggal_pemesanan[${index}]:`,
              data.formatted_tanggal_pemesanan
            );
          });
